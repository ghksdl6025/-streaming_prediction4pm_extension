...
iro1_df = iro1_df.reset_index(drop=True)
oir1_df = oir1_df.reset_index(drop=True)

baseline_df.loc[:,'Complete Timestamp'] = pd.to_datetime(baseline_df['Complete Timestamp'])


iro1_df.loc[:,'Complete Timestamp'] = pd.to_datetime(iro1_df['Complete Timestamp'])


oir1_df.loc[:,'Complete Timestamp'] = pd.to_datetime(oir1_df['Complete Timestamp'])


for pos, t in enumerate(list(baseline_df['Activity'])):
    if t == 'Loan__application_approved':
        baseline_df.loc[pos, 'Complete Timestamp'] = baseline_df.loc[pos, 'Complete Timestamp'] + pd.Timedelta(minutes=1)

    elif t == 'Loan_application_rejected':
        baseline_df.loc[pos, 'Complete Timestamp'] = baseline_df.loc[pos, 'Complete Timestamp'] + pd.Timedelta(minutes=1)
    
    elif t == 'Loan__application_canceled':
        baseline_df.loc[pos, 'Complete Timestamp'] = baseline_df.loc[pos, 'Complete Timestamp'] + pd.Timedelta(minutes=1)
          
for pos, t in enumerate(list(iro1_df['Activity'])):
    if t == 'Loan__application_approved':
        iro1_df.loc[pos, 'Complete Timestamp'] = iro1_df.loc[pos, 'Complete Timestamp'] + pd.Timedelta(minutes=1)
...
for pos, t in enumerate(list(oir1_df['Activity'])):
    if t == 'Loan__application_approved':
        oir1_df.loc[pos, 'Complete Timestamp'] = oir1_df.loc[pos, 'Complete Timestamp'] + pd.Timedelta(minutes=1)
